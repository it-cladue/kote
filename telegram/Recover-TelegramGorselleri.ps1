<#
.SYNOPSIS
    Telegram botunun GECMISTE aldigi ekran goruntulerini, Gyazo'ya hic dokunmadan dogrudan
    TELEGRAM'dan geri indirir. Gyazo'daki eski linkler olse bile calisir.

.DESCRIPTION
    Bot her gorseli Gyazo'ya yukleyip Excel'e link yaziyordu; gorselin kendisini yerelde tutmuyordu.
    Excel'de/logda Telegram dosya kimligi (file_id) YOK. Elimizdeki tek Telegram izi telegram_cache.txt:
    her satir "<kullaniciID>:<mesajID>" (ozel sohbette sohbet ID'si = kullanici ID'sidir).

    Telegram Bot API bir botun eski mesajlari ID ile "okumasina" izin vermez; ama mesaji BASKA bir
    sohbete YONLENDIRMEYE (forwardMessage) izin verir ve yonlendirme sonucu mesajin kendisini
    (fotograf + orijinal tarih) doner. Bu script her cache kaydini bir "dokme" sohbetine yonlendirir,
    fotograf ise en buyuk boyutu getFile ile indirir, sonra yonlendirilen kopyayi siler.

    Adlandirma: orijinal gonderim tarihi + kullanici (cache'deki sohbet ID'si GERCEK kullanicidir).
    Ek olarak, bot_log.txt + Excel ile eslestirerek (ayni kullanici + yakin zaman) proje / kategori /
    personel / uye ID bilgisini de dosya adina ekler (-Zenginlestir; bulunamazsa bos birakir).

    ONEMLI:
      * Bu script CANLI bot token'ini kullanir; sadece sizin makinenizde, bot DURUYORKEN calistirin
        (calisan bot ayni getUpdates'i cekmesin diye). Token config.json'dan okunur, hicbir yere yazilmaz.
      * forwardMessage her mesaji -HedefChatId sohbetine bir an icin dusurur; script hemen siler.
        Cok sayida bildirim olmasin diye HedefChatId olarak BOT'un yonetici oldugu bos bir grup/kanal
        verin (bkz. README). Vermezseniz config.json'daki ilk ADMIN_IDS kullanilir.
      * Silinmis mesajlar (bot akisindaki personel/uye ID yazilari bot tarafindan silinir) yonlendirilemez;
        bunlar "mesaj_yok" olarak atlanir. Fotograf mesajlari silinmedigi icin kurtarilabilir.
      * Telegram eski mesajlari suresiz tutar; yine de kullanici sohbeti sildiyse/botu engellediyse
        o mesajlar gelmez.

    Verimlilik: cache kronolojiktir. -SonAy 2 ile SADECE son 2 ay istendiginde script cache'i SONDAN
    (en yeni) basa dogru tarar ve tarih araligindan yeterince eskiye inince durur; boylece tum gecmisi
    yonlendirmez.

.PARAMETER BotKlasoru
    telegram_cache.txt, config.json, bot_log.txt, telegrambot_gyazo.xlsx vs. klasoru. Varsayilan: script klasoru.
.PARAMETER Token
    Telegram bot token. Varsayilan: config.json -> TELEGRAM_TOKEN.
.PARAMETER HedefChatId
    Mesajlarin bir an icin yonlendirilecegi sohbet. Varsayilan: config.json -> ADMIN_IDS[0].
    ONERI: bot'un yonetici oldugu, sadece bunun icin acilmis bos bir grup/kanal ID'si.
.PARAMETER CiktiKlasoru
    Varsayilan: <BotKlasoru>\telegram_gorseller
.PARAMETER SonAy
    Kac ay geriye. Varsayilan 2. -Baslangic verilirse yok sayilir.
.PARAMETER Baslangic / Bitis
    Tarih araligi (orijinal gonderim tarihine gore). Orn. "2026-07-15".
.PARAMETER AdSablonu
    Alanlar: {tarih} {kullanici} {kullaniciid} {proje} {kategori} {personel} {uyeid} {anaid} {tip} {mesajid}
.PARAMETER Zenginlestir
    Log+Excel ile proje/kategori/personel/uye bilgisini dosya adina ekle (varsayilan acik). Kapatmak: -Zenginlestir:$false
.PARAMETER SadeceTara
    Indirme yok; sadece hangi mesajlar fotograf, indeks.csv olustur (yine de forwardMessage yapar).
.PARAMETER Yeniden
    Daha once inmis dosyalari da yeniden indir.
.PARAMETER KopyaBirak
    Yonlendirilen kopyayi hedef sohbetten SILME, birak (varsayilan: siler).
.PARAMETER EskiDurma
...
.EXAMPLE
    # Once dene: son 2 ayi tara, indirme yok
...
.NOTES
    Windows PowerShell 5.1 ve PowerShell 7 uyumlu. Bot DURUYORKEN calistirin.
#>
[CmdletBinding()]
param(
...
Write-Bilgi "========================================"
Write-Bilgi "Telegram gorsel kurtarma basliyor"
